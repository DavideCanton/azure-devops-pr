...
import { Comment, CommentThreadStatus, CommentType, GitPullRequest, GitPullRequestCommentThread } from "azure-devops-node-api/interfaces/GitInterfaces";
...
export interface AzureClient {
    ...
    comment(content: string, pullRequestId: number, threadId: number, parentCommentId: number | null): Promise<Comment>;
    createThread(pullRequestId: number, content: string): Promise<GitPullRequestCommentThread>;
}
...
class AzureRealClient implements AzureClient {
    ...
    async createThread(pullRequestId: number, content: string): Promise<GitPullRequestCommentThread> {
        await this.loadClient();
    ...
    async comment(content: string, pullRequestId: number, threadId: number, parentCommentId: number | null = null): Promise<Comment> {
        await this.loadClient();

        return await this.gitClient!.createComment(
            this._createComment(content, parentCommentId),
            this.configuration.repositoryName,
            pullRequestId, threadId, this.configuration.projectName
        );
    }

    private _createComment(content: string, parentCommentId: number | null = null): Comment {
        return {
            author: {},
...
class MockClient implements AzureClient {
    ...
    async comment(content: string, pullRequestId: number, threadId: number, parentCommentId: number | null): Promise<Comment> {
        return {} as Comment;
    }
